...
main :: IO ()
main = do
    --patchFileMainForRunForAllForTime ( runForAllAssocCollectionImplementationsSourceFilesPath ++ "Main.hs.template" )
    
    SP.callCommand "cabal configure --ghc-options=-O0"
    
    SP.callCommand "cabal build runForAllAssocCollectionImplementations --ghc-options='-O0 -fforce-recomp'"

    ( _ , Just hout , _ , _ ) <- SP.createProcess ( ( SP.proc "./dist/build/runForAllAssocCollectionImplementations/runForAllAssocCollectionImplementations" [] ){ SP.std_out = SP.CreatePipe } )
    ...
    patchFileMainForRunForAllForEnergy ( runForAllAssocCollectionImplementationsSourceFilesPath ++ "Main.hs.template" )
    
    SP.callCommand "cabal configure"
    
    SP.callCommand "cabal build runForAllAssocCollectionImplementations"
...
patchFileMainForRunForAllForEnergy f = do
    ...
    let
        ...
        io2 = DTL.replace ( DTL.pack "<benchmarkIdPrefix>" ) ( DTL.pack "\"EnergyConsumption/AssocCollection\"") io1 
...
patchFileMainForRunForAllForTime f = do
    ...
    let
        ...
        io2 = DTL.replace ( DTL.pack "<benchmarkIdPrefix>" ) ( DTL.pack "\"Time/AssocCollection\"") io1 
