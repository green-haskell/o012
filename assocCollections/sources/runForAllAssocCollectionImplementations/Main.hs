...
main = do
    ...
    packageEnergyOutputCSV <- processFeature "packageEnergy" benchmarksOutputsForAllAssocCollectionImplementations
    DTLI.writeFile "packageEnergy-Energy.csv" packageEnergyOutputCSV :: IO ()

    dramEnergyOutputCSV <- processFeature "dramEnergy" benchmarksOutputsForAllAssocCollectionImplementations
...
processFeature feature benchmarksOutputsForAllAssocCollectionImplementations = do
    let
        values = case feature of
            ...
            "packageEnergy" -> map ( map getPackageEnergy ) benchmarksOutputsForAllAssocCollectionImplementations
            "dramEnergy" -> map ( map getDRAMEnergy ) benchmarksOutputsForAllAssocCollectionImplementations
                :: [ [ ( DTL.Text , Double , DTL.Text ) ] ]
    --print values
...
getPackageEnergy bmOutput =
    let
        ...
        bmName = DTL.append ( DTL.pack "PackageEnergyConsumption/" ) . snd . DTL.breakOn ( DTL.pack "AssocCollection/" ) . head . take 1 $ ls
...
getDRAMEnergy bmOutput =
    let
        ...
        bmName = DTL.append ( DTL.pack "DRAMEnergyConsumption/" ) . snd . DTL.breakOn ( DTL.pack "AssocCollection/" ) . head . take 1 $ ls
