...
assocCollectionImplementations :: [ String ]
...
readFiles feature = do
    let
        ...
        filenames = map ( (++) csvFileSetsFilesPath . flip (++) ( '.' : ( feature ++ ".csv" ) ) ) assocCollectionImplementations
            :: [ FilePath ]
    --print filenames
    ...
    let
        simplifiedFilesContentsAsLines = map (
            ...
            DTL.replace ( DTL.pack (
                case feature of
                    "time" -> "Time/AssocCollection/Data.Edison.Assoc."
                    --"energy" -> "EnergyConsumption/AssocCollection/Data.Edison.Assoc."
                    "packageEnergy" -> "PackageEnergyConsumption/AssocCollection/Data.Edison.Assoc."
                    "dramEnergy" -> "DRAMEnergyConsumption/AssocCollection/Data.Edison.Assoc."
...
finishResults feature ir1 =
    ...
    . map (
        ...
        -- mantain the assocCollectionID and separate the benchmarkId from the results
        . (><) id ( DTL.span ( (/=) '\"' ) . DTL.tail )
        -- separate the assocCollectionID from the rest
        . DTL.span ( (/=) '/' )
        )
        $ ir1
