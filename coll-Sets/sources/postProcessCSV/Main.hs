...
readFiles feature = do
    let
        -- NOTE: Set Implementations HARDWIRED
        filenames = map ( (++) csvFileSetsFilesPath . flip (++) ( '.' : ( feature ++ ".csv" ) ) ) setImplementations
            :: [ FilePath ]
    --print filenames
    ...
    let
        simplifiedFilesContentsAsLines = map (
            ...
            DTL.replace ( DTL.pack (
                case feature of
                    "time" -> "Time/Set/Data.Edison.Coll."
                    --"energy" -> "EnergyConsumption/Set/Data.Edison.Coll."
                    "packageEnergy" -> "PackageEnergyConsumption/Set/Data.Edison.Coll."
                    "dramEnergy" -> "DRAMEnergyConsumption/Set/Data.Edison.Coll."
...
finishResults feature ir1 =
    ...
    . map (
        ...
        -- mantain the setID and separate the benchmarkId from the results
        . (><) id ( DTL.span ( (/=) '\"' ) . DTL.tail )
        -- separate the setID from the rest
        . DTL.span ( (/=) '/' )
        )
        $ ir1
