...
main = do
    ...
    packageEnergyOutputCSV <- processFeature "packageEnergy" benchmarksOutputsForAllSetImplementations
    DTLI.writeFile "packageEnergy-Energy.csv" packageEnergyOutputCSV :: IO ()

    dramEnergyOutputCSV <- processFeature "dramEnergy" benchmarksOutputsForAllSetImplementations
...
processFeature feature benchmarksOutputsForAllSetImplementations = do
    let
        values = case feature of
            ...
            "packageEnergy" -> map ( map getPackageEnergy ) benchmarksOutputsForAllSetImplementations
            "dramEnergy" -> map ( map getDRAMEnergy ) benchmarksOutputsForAllSetImplementations            
                :: [ [ ( DTL.Text , Double , DTL.Text ) ] ]
    --print values
...
getPackageEnergy bmOutput =
    let
        ...
        bmName = DTL.append ( DTL.pack "PackageEnergyConsumption/" ) . snd . DTL.breakOn ( DTL.pack "Set/" ) . head . take 1 $ ls
...
getDRAMEnergy bmOutput =
    let
        ...
        bmName = DTL.append ( DTL.pack "DRAMEnergyConsumption/" ) . snd . DTL.breakOn ( DTL.pack "Set/" ) . head . take 1 $ ls
