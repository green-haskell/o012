...
main :: IO ()
main = do
    --patchFileMainForRunForAllForTime ( runForAllSetImplementationsSourceFilesPath ++ "Main.hs.template" )
    
    SP.callCommand "cabal configure --ghc-options=-O0"
    
    SP.callCommand "cabal build runForAllSetImplementations  --ghc-options='-O0 -fforce-recomp'"

    ( _ , Just hout , _ , _ ) <- SP.createProcess ( ( SP.proc "./dist/build/runForAllSetImplementations/runForAllSetImplementations" [] ){ SP.std_out = SP.CreatePipe } )
    ...
    patchFileMainForRunForAllForEnergy ( runForAllSetImplementationsSourceFilesPath ++ "Main.hs.template" )
    
    SP.callCommand "cabal configure"
    
    SP.callCommand "cabal build runForAllSetImplementations"
...
patchFileMainForRunForAllForEnergy f = do
    ...
    let
        ...
        io2 = DTL.replace ( DTL.pack "<benchmarkIdPrefix>" ) ( DTL.pack "\"EnergyConsumption/Set\"") io1 
...
patchFileMainForRunForAllForTime f = do
    ...
    let
        ...
        io2 = DTL.replace ( DTL.pack "<benchmarkIdPrefix>" ) ( DTL.pack "\"Time/Set\"") io1 
