...
main = do
    ...
    packageEnergyOutputCSV <- processFeature "packageEnergy" benchmarksOutputsForAllSequenceImplementations
    DTLI.writeFile "packageEnergy-Energy.csv" packageEnergyOutputCSV :: IO ()

    dramEnergyOutputCSV <- processFeature "dramEnergy" benchmarksOutputsForAllSequenceImplementations
...
processFeature feature benchmarksOutputsForAllSequenceImplementations = do
    let
        values = case feature of
            ...
            "packageEnergy" -> map ( map getPackageEnergy ) benchmarksOutputsForAllSequenceImplementations
            "dramEnergy" -> map ( map getDRAMEnergy ) benchmarksOutputsForAllSequenceImplementations
                :: [ [ ( DTL.Text , Double , DTL.Text ) ] ]
    --print values
...
getPackageEnergy bmOutput =
    let
        ...
        bmName = DTL.append ( DTL.pack "PackageEnergyConsumption/" ) . snd . DTL.breakOn ( DTL.pack "Sequence/" ) . head . take 1 $ ls
...
getDRAMEnergy bmOutput =
    let
        ...
        bmName = DTL.append ( DTL.pack "DRAMEnergyConsumption/" ) . snd . DTL.breakOn ( DTL.pack "Sequence/" ) . head . take 1 $ ls
