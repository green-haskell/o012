...
readFiles feature = do
    let
        -- NOTE: Heap Implementations HARDWIRED
        filenames = map ( (++) csvFileSetsFilesPath . flip (++) ( '.' : ( feature ++ ".csv" ) ) ) heapImplementations
            :: [ FilePath ]
    --print filenames
    ...
    let
        simplifiedFilesContentsAsLines = map (
            ...
            DTL.replace ( DTL.pack (
                case feature of
                    "time" -> "Time/Heap/Data.Edison.Coll."
                    --"energy" -> "EnergyConsumption/Heap/Data.Edison.Coll."
                    "packageEnergy" -> "PackageEnergyConsumption/Heap/Data.Edison.Coll."
                    "dramEnergy" -> "DRAMEnergyConsumption/Heap/Data.Edison.Coll."
...
finishResults feature ir1 =
    ...
    . map (
        ...
        -- mantain the heapID and separate the benchmarkId from the results
        . (><) id ( DTL.span ( (/=) '\"' ) . DTL.tail )
        -- separate the heapID from the rest
        . DTL.span ( (/=) '/' )
        )
        $ ir1
