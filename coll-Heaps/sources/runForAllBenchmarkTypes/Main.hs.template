...
main :: IO ()
main = do
    --patchFileMainForRunForAllForTime ( runForAllHeapImplementationsSourceFilesPath ++ "Main.hs.template" )
    
    SP.callCommand "cabal configure --ghc-options=-O0"
    
    SP.callCommand "cabal build runForAllHeapImplementations --ghc-options='-O0 -fforce-recomp'"

    ( _ , Just hout , _ , _ ) <- SP.createProcess ( ( SP.proc "./dist/build/runForAllHeapImplementations/runForAllHeapImplementations" [] ){ SP.std_out = SP.CreatePipe } )
    ...
    patchFileMainForRunForAllForEnergy ( runForAllHeapImplementationsSourceFilesPath ++ "Main.hs.template" )
    
    SP.callCommand "cabal configure"
    
    SP.callCommand "cabal build runForAllHeapImplementations"
...
patchFileMainForRunForAllForEnergy f = do
    ...
    let
        ...
        io2 = DTL.replace ( DTL.pack "<benchmarkIdPrefix>" ) ( DTL.pack "\"EnergyConsumption/Heap\"") io1 
...
patchFileMainForRunForAllForTime f = do
    ...
    let
        ...
        io2 = DTL.replace ( DTL.pack "<benchmarkIdPrefix>" ) ( DTL.pack "\"Time/Heap\"") io1 
