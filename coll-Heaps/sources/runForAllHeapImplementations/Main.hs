...
main = do
    ...
    packageEnergyOutputCSV <- processFeature "packageEnergy" benchmarksOutputsForAllHeapImplementations
    DTLI.writeFile "packageEnergy-Energy.csv" packageEnergyOutputCSV :: IO ()

    dramEnergyOutputCSV <- processFeature "dramEnergy" benchmarksOutputsForAllHeapImplementations
...
processFeature feature benchmarksOutputsForAllHeapImplementations = do
    let
        values = case feature of
            ...
            "packageEnergy" -> map ( map getPackageEnergy ) benchmarksOutputsForAllHeapImplementations
            "dramEnergy" -> map ( map getDRAMEnergy ) benchmarksOutputsForAllHeapImplementations
                :: [ [ ( DTL.Text , Double , DTL.Text ) ] ]
    --print values
...
getPackageEnergy bmOutput =
    let
        ...
        bmName = DTL.append ( DTL.pack "PackageEnergyConsumption/" ) . snd . DTL.breakOn ( DTL.pack "Heap/" ) . head . take 1 $ ls
...
getDRAMEnergy bmOutput =
    let
        ...
        bmName = DTL.append ( DTL.pack "DRAMEnergyConsumption/" ) . snd . DTL.breakOn ( DTL.pack "Heap/" ) . head . take 1 $ ls
